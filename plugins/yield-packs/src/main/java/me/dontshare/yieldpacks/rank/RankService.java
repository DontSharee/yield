...
/**
 * Rank cost curve, the gem-income bonus it grants, and the actual purchase
 ...
 * pattern), except the currency is gems, not coins, and there's no reset of
 * anything - rank just keeps climbing forever.
 */
public final class RankService {

    /** Same reasoning as RebirthService's own cap - coins/gems are unbounded (BigInteger), so a misconfigured near-1.0 growth rate against a huge balance could otherwise loop essentially forever. */
    private static final int MAX_RANKS_PER_PREVIEW = 100_000;

    private static final long BASE_COST_GEMS = 1_000L;
    private static final double COST_GROWTH = 1.15;
    /** +2% gems per rank, additive - same shape every other "1.0 + Σ" multiplier in this codebase uses. */
    private static final double GEM_BONUS_PER_RANK = 0.02;
    ...
    /** {@code BASE_COST_GEMS * COST_GROWTH^rank} - the gem cost to go from {@code rank} to {@code rank + 1}. */
    public BigInteger costForRank(int rank) {
        BigDecimal cost = BigDecimal.valueOf(BASE_COST_GEMS).multiply(BigDecimal.valueOf(COST_GROWTH).pow(rank));
        return cost.setScale(0, RoundingMode.HALF_UP).toBigInteger();
    }

    /** The permanent gem-income multiplier from rank - consumed via YieldPacks#gemMultiplier's own composable registry. */
    public double gemMultiplier(PackPlayerProfile profile) {
        return gemMultiplierForRank(profile.getRank());
    }

    /** Same formula as {@link #gemMultiplier}, for an arbitrary rank rather than a real profile - lets RankupGui show what a NOT-YET-owned rank slot would grant without needing a fake profile. */
    public double gemMultiplierForRank(int rank) {
        return 1.0 + rank * GEM_BONUS_PER_RANK;
    }
    ...
    public RankPreview preview(PackPlayerProfile profile) {
        ...
        BigInteger remaining = profile.getGems();
    ...
    public RankPreview rankUpMax(Player player) {
        ...
        if (preview.available() > 0) {
            profile.setGems(profile.getGems().subtract(preview.totalCost()));
    ...
    public RankPreview rankUpTo(Player player, int targetRank) {
        ...
        if (profile.getGems().compareTo(totalCost) < 0) {
            return new RankPreview(0, BigInteger.ZERO);
        }
        int granted = targetRank - profile.getRank();
        profile.setGems(profile.getGems().subtract(totalCost));
