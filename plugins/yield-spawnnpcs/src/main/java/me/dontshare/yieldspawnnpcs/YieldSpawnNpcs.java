 ...
 * CrateService} for why this is a fully separate config/content rather
 * than reusing that plugin's own). NPCs themselves are NOT built here -
 * FancyNpcs (already installed on this server) handles those directly:
 * create an NPC in-game (see FancyNpcs' own "/npc create" command), then
 * attach a right-click "PLAYER_COMMAND" action running "/rankup", "/buy",
 * or "/crates" - no custom Java needed for any of the three, since all
 * three are already real, standalone player commands.
 */
