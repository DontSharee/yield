...
public final class DonorRankService {
    ...
    public DonorRankService(Supplier<Map<String, DonorRank>> ranks, PlayerDataStore<PackPlayerProfile> store) {
        this.ranks = ranks;
        this.store = store;
    }

    public Optional<DonorRank> currentRank(PackPlayerProfile profile) {
        String id = profile.getDonorRankId();
    ...
    public double xpMultiplier(PackPlayerProfile profile) {
        return currentRank(profile).map(DonorRank::xpMultiplier).orElse(1.0);
    }

    public double luckBonus(PackPlayerProfile profile) {
        return currentRank(profile).map(DonorRank::luckBonus).orElse(0.0);
    }

    public int bonusPetSlots(PackPlayerProfile profile) {
        return currentRank(profile).map(DonorRank::bonusPetSlots).orElse(0);
    }

    public int bonusEnchantSlots(PackPlayerProfile profile) {
        return currentRank(profile).map(DonorRank::bonusEnchantSlots).orElse(0);
    }
    ...
    public void applyRank(Player player, String rankId) {
        ...
        Optional<DonorRank> existing = currentRank(profile);
